...
ggplot(sdr_data, aes(x = goal_4_score, fill=regions_used_for_the_sdr)) +
  ...
  labs(title = "Distributions of SDG 4 Scores",
       x = "SDG 4 Score - Quality Education",
...
```{r}
goal_3_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point () +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "Scatterplot of SDG 3 and SDG 4 Scores",
       x = "SDG 3 Score for Good Health & Well-Being",
       y = "SDG 4 Score for Quality Education",
       fill = "Region")

ggplotly(goal_3_scatterplot)
```

```{r}
scatterplot_3_4 <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point()+
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "Scatterplot of Scores on Goal 3 and 4",
       x = "SDG 3 Score - Good Health & Well-Being",
       y = "SDG 4 Score - Quality Education",
       fill = "Region")

ggplotly(scatterplot_3_4)
```

INTERPRETATION: This new scatterplot looks so much better. One thing I would be interested in is how to change the names of the columns (it would be nice to clean up the legend heading as an example). 
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Oranges", goal_1_score)(goal_1_score), label = mytext1)
```

INTERPRETATION: Awesome! Really enjoyed this one and I can tell that I am figuring out the code! YEAH!
